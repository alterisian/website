...
impl FileSpec {
    ...
    pub(crate) fn generate(
        ...
        async move {
            ...
            let file_contents = task.await.map_err(|error| {
            ...
            file_handle
                .write_all(file_contents.bytes())
                .await
                .map_err(|error| {
                    TargetError::new(this_target.clone(), TargetErrorCause::TargetIo(error))
                })?;

            let expected_targets = file_contents
                .expected_targets()
                .cloned()
                .unwrap_or_default();

            Ok(TargetSuccess::new(this_target, Some(expected_targets)))
        }
        .boxed()
    }
}
